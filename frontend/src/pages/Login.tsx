...
export default function Login() {
  ...
  const handleGoogle = async () => {
    ...
    const { error } = await supabase.auth.signInWithOAuth({
      provider: 'google',
      options: { redirectTo: `${window.location.origin}/hoy` },
    });
    if (error) {
      setError(translateError(error.message));
      ...
      <div style={{ width: '100%', maxWidth: '420px' }}>
        ...
        <div style={{
          ...
          <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            ...
            <button type="submit" disabled={loading || googleLoading} className="nb-btn-primary" style={{ marginTop: '4px' }}>
          ...
          <button onClick={handleGoogle} disabled={loading || googleLoading} className="nb-btn-google">
            <GoogleIcon />
