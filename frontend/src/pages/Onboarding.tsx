...
const STEP_LABELS = ['Datos personales', 'Tu cuerpo', 'Tu objetivo', 'Horarios', 'Tu salud'];
...
// Inputs de hora por número de comidas
type CampoHorario = keyof Pick<FormState,
    ...
    <div>
      ...
      <div className="grid grid-cols-2 gap-4 mt-4">
        ...
        <div>
          ...
          <input type="text" value={form.ciudad} onChange={set('ciudad')} placeholder="Tu ciudad"
            className="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-green-400" />
        </div>
      </div>
    </div>
...
function Step2({ form, setForm }: { form: FormState; setForm: React.Dispatch<React.SetStateAction<FormState>> }) {
  ...
  const imc = !isNaN(peso) && !isNaN(estatura) && estatura > 0 ? calcularIMC(peso, estatura) : null;
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 gap-4 onboarding-grid-2">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1.5">Estatura (cm) *</label>
          <input type="number" value={form.estatura_cm} onChange={set('estatura_cm')} required
      ...
      {imc && catIMC && (
        <div className="mt-4 px-4 py-3 rounded-xl flex items-center justify-between"
          ...
          <div>
            ...
            <p className="text-2xl font-bold" style={{ color: catIMC.color }}>{imc}</p>
          </div>
          <div className="px-3 py-1 rounded-full text-sm font-medium text-white" style={{ backgroundColor: catIMC.color }}>
            {catIMC.label}
          </div>
        </div>
      )}
        ...
        <div className="flex flex-col gap-2">
          {ACTIVIDADES.map(({ value, label, desc }) => (
            <label key={value}
              ...
              <input type="radio" name="nivel_actividad" value={value}
...
function Step3({ form, setForm }: { form: FormState; setForm: React.Dispatch<React.SetStateAction<FormState>> }) {
  return (
    <div>
      ...
      <p className="text-sm text-gray-500 mb-6">Ajustamos tus calorías y macros para alcanzarlo.</p>
        ...
        {OBJETIVOS.map(({ value, emoji, label, desc }) => (
          <button key={value} type="button"
            ...
            <div className="flex-1">
              <p className="text-sm font-semibold" style={{ color: form.objetivo === value ? '#085041' : '#374151' }}>
                {label}
              </p>
            ...
            {form.objetivo === value && (
              <div className="w-5 h-5 rounded-full flex items-center justify-center text-white text-xs shrink-0"
                style={{ backgroundColor: '#1D9E75' }}>✓</div>
            )}
          </button>
        ))}
      </div>
...
// ──────────────────────────────────────────────
// Paso 4 (NUEVO): Horarios de comida
// ──────────────────────────────────────────────

function Step4Horarios({ form, setForm }: { form: FormState; setForm: React.Dispatch<React.SetStateAction<FormState>> }) {
  const recomendado = recomendarComidas(form.objetivo, form.nivel_actividad);
  const inputs = HORARIOS_POR_COMIDAS[form.num_comidas_dia as 3 | 4 | 5] ?? HORARIOS_POR_COMIDAS[5];

  const setHora = (campo: CampoHorario) => (e: React.ChangeEvent<HTMLInputElement>) =>
    setForm(f => ({ ...f, [campo]: e.target.value }));

  const setNumComidas = (n: number) => {
    setForm(f => ({ ...f, num_comidas_dia: n }));
  };

  const OPCIONES_COMIDAS: { n: 3 | 4 | 5; desc: string }[] = [
    { n: 3, desc: 'Desayuno · Comida · Cena' },
    { n: 4, desc: 'Desayuno · Comida · Merienda · Cena' },
    { n: 5, desc: 'Desayuno · 2 colaciones · Comida · Cena' },
  ];

  return (
    <div>
      <h3 className="text-lg font-semibold text-gray-800 mb-1">Tus horarios de comida</h3>
      <p className="text-sm text-gray-500 mb-4">
        Organiza tus comidas para mantener niveles de energía estables.
      </p>

      {/* Chip de recomendación */}
      <div className="flex items-center gap-2 px-4 py-2.5 rounded-xl mb-5 w-fit"
        style={{ backgroundColor: '#E1F5EE', border: '1px solid #A7E3C8' }}>
        <span className="text-base">✨</span>
        <span className="text-sm font-medium" style={{ color: '#085041' }}>
          Recomendamos <strong>{recomendado} comidas</strong> para tu objetivo
        </span>
      </div>

      {/* Selector de número de comidas */}
      <div className="flex flex-col gap-2 mb-6">
        {OPCIONES_COMIDAS.map(({ n, desc }) => (
          <button key={n} type="button"
            ...
            style={{
              borderColor:     form.num_comidas_dia === n ? '#1D9E75' : '#E5E7EB',
              backgroundColor: form.num_comidas_dia === n ? '#F0FBF7' : '#fff',
            }}>
            <div>
              <span className="text-sm font-semibold text-gray-800">{n} comidas al día</span>
              <p className="text-xs text-gray-500 mt-0.5">{desc}</p>
            </div>
            {form.num_comidas_dia === n && (
              <div className="w-5 h-5 rounded-full flex items-center justify-center text-white text-xs shrink-0"
                style={{ backgroundColor: '#1D9E75' }}>✓</div>
            )}
          </button>
      ...
      {/* Inputs de hora dinámicos */}
      <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">
        Ajusta tus horarios
      </p>
      <div className="grid grid-cols-2 gap-3">
        {inputs.map(({ campo, label, emoji }) => (
          <div key={campo}>
            <label className="flex items-center gap-1.5 text-sm font-medium text-gray-700 mb-1.5">
              <span>{emoji}</span> {label}
            </label>
            <input type="time" value={form[campo] as string} onChange={setHora(campo)}
...
// ──────────────────────────────────────────────
// Paso 5: Salud y preferencias (antes Step4)
// ──────────────────────────────────────────────

function Step5Salud({ form, setForm }: { form: FormState; setForm: React.Dispatch<React.SetStateAction<FormState>> }) {
  const toggleCondicion = (key: keyof CondicionesMedicas) =>
    setForm(f => ({ ...f, condiciones_medicas: { ...f.condiciones_medicas, [key]: !f.condiciones_medicas[key] } }));

  const togglePreferencia = (key: keyof PreferenciasAlimentarias) =>
    setForm(f => ({ ...f, preferencias_alimentarias: { ...f.preferencias_alimentarias, [key]: !f.preferencias_alimentarias[key] } }));
  ...
  const Checkbox = ({ checked, label, onChange }: { checked: boolean; label: string; onChange: () => void }) => (
    <label
      ...
      style={{ borderColor: checked ? '#1D9E75' : '#E5E7EB', backgroundColor: checked ? '#F0FBF7' : '#FAFAFA' }}>
      <div className="w-4 h-4 rounded flex items-center justify-center shrink-0 border transition-all"
        style={{ borderColor: checked ? '#1D9E75' : '#D1D5DB', backgroundColor: checked ? '#1D9E75' : '#fff' }}>
        {checked && (
  ...
  return (
    <div>
      ...
      <p className="text-sm text-gray-500 mb-6">Adaptamos las recetas a tus necesidades. Selecciona todo lo que aplique.</p>

      <div className="mb-6">
        <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">Condiciones médicas</p>
        <div className="grid grid-cols-2 gap-2">
          {CONDICIONES.map(({ key, label }) => (
            <Checkbox key={key} checked={!!form.condiciones_medicas[key]} label={label}
      ...
      <div className="mb-5">
        <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">Preferencias alimentarias</p>
        <div className="grid grid-cols-2 gap-2">
          {PREFERENCIAS.map(({ key, label }) => (
            <Checkbox key={key} checked={!!form.preferencias_alimentarias[key]} label={label}
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1.5">Otras restricciones o alergias</label>
        <input type="text" value={form.otras_restricciones}
...
export default function Onboarding() {
  ...
  useEffect(() => {
    ...
    setForm(f => ({
      ...
      num_comidas_dia:      perfil?.num_comidas_dia          ?? f.num_comidas_dia,
      horario_desayuno:         perfil?.horario_desayuno         || f.horario_desayuno,
      horario_colacion_manana:  perfil?.horario_colacion_manana  || f.horario_colacion_manana,
      horario_comida:           perfil?.horario_comida           || f.horario_comida,
      horario_merienda:         perfil?.horario_merienda         || f.horario_merienda,
      horario_colacion_tarde:   perfil?.horario_colacion_tarde   || f.horario_colacion_tarde,
      horario_cena:             perfil?.horario_cena             || f.horario_cena,
  ...
  const isStepValid = (): boolean => {
    ...
    return true; // pasos 3, 4 y 5 siempre son válidos (tienen defaults)
  };
  ...
  const handleFinish = async () => {
    ...
    try {
      await guardarPerfil({
        ...
        num_comidas_dia:      form.num_comidas_dia,
        horario_desayuno:         form.horario_desayuno,
        horario_colacion_manana:  form.horario_colacion_manana,
        horario_comida:           form.horario_comida,
        horario_merienda:         form.horario_merienda,
        horario_colacion_tarde:   form.horario_colacion_tarde,
        horario_cena:             form.horario_cena,
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 py-8" style={{ backgroundColor: '#F8FDFB' }}>
      <div className="w-full max-w-xl">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
          ...
          {step === 4 && <Step4Horarios form={form} setForm={setForm} />}
          {step === 5 && <Step5Salud form={form} setForm={setForm} />}
